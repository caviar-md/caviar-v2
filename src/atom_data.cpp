...
namespace caviar2
{
  ...
  Atom_data::Atom_data(Caviar2 *fptr) : caviar_{fptr},
                                        //  num_local_atoms{0},
  ...
  bool Atom_data::read(caviar2::interpreter::Parser *parser)
  {
    ...
    while (true)
    {
      ...
      if (string_cmp(t, "ghost_cutoff"))
      {
        ...
        if (ghost_cutoff < 0.0)
          caviar_->log.error_all(FC_FILE_LINE_FUNC_PARSE, "ghost_cutoff have to non-negative.");
      }
      else if (string_cmp(t, "neighborlist_cutoff"))
      {
        GET_OR_CHOOSE_A_REAL(neighborlist_cutoff, "", "")
        if (neighborlist_cutoff < 0.0)
          caviar_->log.error_all(FC_FILE_LINE_FUNC_PARSE, "neighborlist_cutoff have to non-negative.");
      }
      ...
      else if (string_cmp(t, "mpi_optimization"))
      {
        ...
        if (t2 == "None")
          ...
          caviar_->log.error_all(FC_FILE_LINE_FUNC_PARSE, "undefined mpi_optimization type: " + t2);
      }
      else if (string_cmp(t, "k_b"))
      {
        GET_OR_CHOOSE_A_REAL(k_b, "", "")
        if (k_b < 0.0)
          caviar_->log.error_all(FC_FILE_LINE_FUNC_PARSE, "k_b have to non-negative.");
      }
      else if (string_cmp(t, "msd_process"))
      {
        GET_OR_CHOOSE_A_INT(msd_process, "", "")
        if (msd_process)
          atom_struct_owned.msd_domain_cross.resize(atom_struct_owned.position.size(), caviar2::Vector3d<int>{0, 0, 0});
      }
      ...
      else if (string_cmp(t, "add_atom"))
      {
        ...
        tools::Atom *a = dynamic_cast<tools::Atom *>(object_container->tools[it->second.index]);
        add_atom(*a);
      }
      else if (string_cmp(t, "add_atom_group"))
      {
        ...
        tools::Atom_group *a = dynamic_cast<tools::Atom_group *>(object_container->tools[it->second.index]);
        add_atom(*a);
      }
      else if (string_cmp(t, "add_atom_list"))
      {
        FIND_OBJECT_BY_NAME(tools, it)
        FC_CHECK_OBJECT_CLASS_NAME(tools, it, atom_list)
        tools::Atom_list *a = dynamic_cast<tools::Atom_list *>(object_container->tools[it->second.index]);
        add_atom(*a);
      }
      else if (string_cmp(t, "add_molecule"))
      {
        ...
        tools::Molecule *a = dynamic_cast<tools::Molecule *>(object_container->tools[it->second.index]);
        add_molecule(*a);
      }
      else if (string_cmp(t, "add_molecule_group"))
      {
        ...
        tools::Molecule_group *a = dynamic_cast<tools::Molecule_group *>(object_container->tools[it->second.index]);
        add_molecule(*a);
      }
      else if (string_cmp(t, "add_molecule_list"))
      {
        FIND_OBJECT_BY_NAME(tools, it)
        FC_CHECK_OBJECT_CLASS_NAME(tools, it, molecule_list)
        tools::Molecule_list *a = dynamic_cast<tools::Molecule_list *>(object_container->tools[it->second.index]);
        add_molecule(*a);
      }
      else if (string_cmp(t, "add_type_radius"))
      {
        ...
        if (r < 0)
          caviar_->log.warning("you have entered a negative value for atom radius.");
      }
      ...
      else if (string_cmp(t, "add_type_mass"))
      {
        ...
        if (m < 0)
          caviar_->log.warning("you have entered a negative value for atom mass.");
      }
  ...
  */
  ...
  // ====================== Cutoff and general parameters ======================
  void Atom_data::set_ghost_cutoff(double value)
  {
    ghost_cutoff = value;
  }

  void Atom_data::set_neighborlist_cutoff(double value)
  {
    neighborlist_cutoff = value;
  }

  void Atom_data::set_cutoff_extra(double value)
  {
    cutoff_extra = value;
  }

  void Atom_data::set_mpi_optimization(MpiOptimization opt)
  {
    mpiOptimization = opt;
  }

  void Atom_data::set_k_b(double value)
  {
    k_b = value;
  }

  void Atom_data::set_msd_process(int flag)
  {
    msd_process = flag;
  }

  void Atom_data::set_debug_virial(int flag)
  {
    debug_virial = flag;
  }

  void Atom_data::set_pressure_process(int flag)
  {
    pressure_process = flag;
  }

  void Atom_data::set_temperature_process(int flag)
  {
    temperature_process = flag;
  }

  // ====================== Type properties ======================
  void Atom_data::add_type_radius(int ind, double r)
  {
    if (static_cast<int>(atom_type_params.radius.size()) < ind + 1)
    {
      atom_type_params.radius.resize(ind + 1);
  ...
  void Atom_data::add_type_charge(int ind, double c)
  {
    if (static_cast<int>(atom_type_params.charge.size()) < ind + 1)
    {
      atom_type_params.charge.resize(ind + 1);
  ...
  void Atom_data::add_type_mass(int ind, double m)
  {
  ...
  void Atom_data::set_acceleration(int ind, const Vector3d<double> &v)
  {
    ...
    if (ind < 0 || ind >= atom_struct_owned.position.size())
      return;
    atom_struct_owned.acceleration[ind] = v;
  }

  // ====================== Misc ======================
  void Atom_data::set_n_r_df(int value)
  {
    n_r_df = value;
  }

  void Atom_data::set_velocity_offset(tools::Time_function_3d *f)
  {
    velocity_offset = f;
  }
  ...
  {
    caviar_->log.error_all(FC_FILE_LINE_FUNC, "Why you need this function??");
  ...
  {
    if (domain == nullptr)
      caviar_->log.error_all(FC_FILE_LINE_FUNC, "domain = nullptr");
  ...
  bool Atom_data::atom_struct_owned_resize(long new_size)
  {
    ...
    atom_struct_owned.position.resize(new_size, caviar2::Vector3d<double>{0, 0, 0});
    atom_struct_owned.velocity.resize(new_size, caviar2::Vector3d<double>{0, 0, 0});
    atom_struct_owned.acceleration.resize(new_size, caviar2::Vector3d<double>{0, 0, 0});
    if (record_owned_position_old)
      atom_struct_owned.position_old.resize(new_size, caviar2::Vector3d<double>{0, 0, 0});
    if (record_owned_velocity_old)
      atom_struct_owned.velocity_old.resize(new_size, caviar2::Vector3d<double>{0, 0, 0});
    if (record_owned_acceleration_old)
      atom_struct_owned.acceleration_old.resize(new_size, caviar2::Vector3d<double>{0, 0, 0});
    if (msd_process)
      atom_struct_owned.msd_domain_cross.resize(new_size);
  ...
  bool Atom_data::atom_struct_ghost_resize(long new_size)
  {
    ...
    atom_struct_ghost.position.resize(new_size, caviar2::Vector3d<double>{0, 0, 0});
    if (make_ghost_velocity)
      atom_struct_ghost.velocity.resize(new_size, caviar2::Vector3d<double>{0, 0, 0});
  ...
  void Atom_data::scale_position(double xi, caviar2::Vector3d<int> scale_axis)
  {
