...
bool Atom_data::add_atom(caviar2::tools::Atom &a)
{
...
bool Atom_data::add_atom(caviar2::tools::Atom_group &ag)
{
    for (auto &&a : ag.atoms)
        add_atom(a);
    return true;
...
bool Atom_data::add_atom(caviar2::tools::Atom_list &al)
{
    for (auto &&a : al.atoms)
        add_atom(*a);
    return true;
...
void Atom_data::remove_atomic_bond(const atom_data::Bond &bond)
{
    ...
    if (mi_1 == mi_2)
    {
        ...
        if (!bond_found)
        {
            caviar_->log.error_all(FC_FILE_LINE_FUNC, "Can't remove bond. It does not exist");
    ...
    {
        caviar_->log.error_all(FC_FILE_LINE_FUNC, "Can't remove atomic bond because the described atoms are of different molecules.");
    }
...
void Atom_data::remove_atomic_angle(const atom_data::Angle &angle)
{
    ...
    {
        caviar_->log.error_all(FC_FILE_LINE_FUNC, "Can't remove atomic angle because the described atoms are of different molecules.");
    }
}
...
void Atom_data::remove_atomic_properdihedral(const atom_data::Proper_dihedral &pd)
{
    ...
    {
        caviar_->log.error_all(FC_FILE_LINE_FUNC, "Can't remove atomic proper dihedral because the described atoms are of different molecules.");
    }
}
...
void Atom_data::add_atomic_angle(const atom_data::Angle &a)
{
    ...
    {
        caviar_->log.error_all(FC_FILE_LINE_FUNC, "New atomic angles must be in the same molecule");
    }
}
...
void Atom_data::add_atomic_properdihedral(const atom_data::Proper_dihedral &pd)
{
    ...
    {
        caviar_->log.error_all(FC_FILE_LINE_FUNC, "New atomic proper dihedrals must be in the same molecule");
    }
}
...
bool Atom_data::add_molecule(caviar2::tools::Molecule &m)
{
...
bool Atom_data::add_molecule(caviar2::tools::Molecule_group &mg)
{
...
bool Atom_data::add_molecule(caviar2::tools::Molecule_list &ml)
{
...
void Atom_data::remove_atom_id_from_molecule(int atom_id, int molecule_index)
{
    ...
    //else
    //    caviar_->log.error_all(FC_FILE_LINE_FUNC, "Can't remove atomic from molecule because it does not exist in the molecule::Atom_list.");
