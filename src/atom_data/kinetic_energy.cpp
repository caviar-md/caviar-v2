 ...
 * -----------------------------------------------------------------------------
 */
#include "caviar2/common_template_functions.hpp"
...
double Atom_data::kinetic_energy_mpi_domain(const int t)
{
  ...
  if (velocity_offset == nullptr)
  {
    ...
    case (6):
    {
      ...
      {
        caviar_->log.warning("Error in matrix_inverse_3d for Atom_data::kinetic_energy.");
        // caviar_->log.error_all(FC_FILE_LINE_FUNC, "Error in Inertia Tensor Inverse Calculations for Atom_data::kinetic_energy.");
      }
...
{
  if (k_b < 0)
    caviar_->log.error_all(FC_FILE_LINE_FUNC, "k_b is not set.");
...
void Atom_data::add_to_external_virial(caviar2::Vector3d<double> force, int i)
{
  virialExternalForce +=
...
void Atom_data::add_to_external_virial(caviar2::Vector3d<double> force, int i, caviar2::Vector3d<double> pos)
{
  virialExternalForce +=
...
void Atom_data::finalize_pressure_mpi_domain()
{
  ...
  //   caviar_->log.warning("Negative pressure");
  // if (finish)
  //   caviar_->log.error_all(FC_FILE_LINE_FUNC, "finish");
}
