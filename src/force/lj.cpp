...
namespace caviar2
{
  ...
  namespace force_field
  {
    ...
    Lj::Lj(Caviar2 *fptr) : Force_field{fptr}
    {
    ...
    /*
      bool Lj::read(caviar2::interpreter::Parser *parser)
      {
        FC_OBJECT_READ_INFO
        bool in_file = true;
    ...
    */
    ...
    void Lj::set_cutoff(double value)
    {
      if (value < 0.0)
        throw std::runtime_error("Force field cutoff must be non-negative.");
      cutoff = value;
    }

    void Lj::set_jump_tol(double value)
    {
      jump_fix = true;
      jump_tol = value;
    }
    ...
    // ---------------- Flags ----------------
    void Lj::enable_monitor_jump(bool flag)
    {
      monitor_jump = flag;
    }

    void Lj::ignore_intra_molecule_flag(bool flag)
    {
      ignore_intra_molecule = flag;
    }

    void Lj::enable_wca(bool flag)
    {
      wca = flag;
      if (flag)
        cutoff_list_activated = true;
    ...
    void Lj::enable_off_diagonal_vectors(bool flag)
    {
      make_off_diagonal_vectors = flag;
    }

    // ---------------- Array / vector parameters ----------------
    void Lj::set_cutoff_list(const std::vector<std::vector<double>> &values)
    {
      for (auto &v : values)
        for (auto val : v)
          if (val < 0.0)
            throw std::runtime_error("Cutoff list values must be non-negative.");
      cutoff_list = values;
      cutoff_list_activated = true;
    }

    void Lj::set_epsilon(const int type1, const int type2, const double value)
    {
      if (epsilon.size() < type1 + 1)
        epsilon.resize(type1 + 1);
      if (epsilon[type1].size() < type2 + 1)
        epsilon[type1].resize(type2 + 1, 0);
      epsilon[type1][type2] = value;
      input_by_array = true;
    }

    void Lj::set_sigma(const int type1, const int type2, const double value)
    {
      if (sigma.size() < type1 + 1)
        sigma.resize(type1 + 1);
      if (sigma[type1].size() < type2 + 1)
        sigma[type1].resize(type2 + 1, 0);
      sigma[type1][type2] = value;

      input_by_array = true;
    }

    void Lj::set_lambda_s(const std::vector<std::vector<double>> &values)
    {
      for (auto &v : values)
        for (auto val : v)
          if (val < 0.0)
            throw std::runtime_error("Lambda_s values must be non-negative.");
      lambda_s = values;
      lambda_s_is_set = true;
    }

    void Lj::set_lambda_e(const std::vector<std::vector<double>> &values)
    {
      for (auto &v : values)
        for (auto val : v)
          if (val < 0.0)
            throw std::runtime_error("Lambda_e values must be non-negative.");
      lambda_e = values;
      lambda_e_is_set = true;
    }

    void Lj::set_epsilon_atom(const int type, const double value)
    {
      if (epsilon_atom.size() < type + 1)
        epsilon_atom.resize(type + 1, 0);
      epsilon_atom[type] = value;
      input_by_atom = true;
    }

    void Lj::set_sigma_atom(const int type, const double value)
    {
      if (sigma_atom.size() < type + 1)
        sigma_atom.resize(type + 1, 0);
      sigma_atom[type] = value;
      input_by_atom = true;
    }
    ...
    void Lj::verify_settings()
    {
      ...
      {
        caviar_->log.error_all(FC_FILE_LINE_FUNC, "(input_by_atom && input_by_array)");
      }
      ...
      if (input_by_atom)
      {
        ...
        {
        caviar_->log.warning("lj:: (epsilon_atom_size != sigma_atom_size)");
        ...
        {
        caviar_->log.warning("lj:: (atom_max_size < atom_data_type_max + 1)"); // if the input is done by epsilon rather than epsilon_atom,
      ...
      else if (input_by_array)
      {
        ...
        {
        caviar_->log.warning("lj:: (epsilon_size != sigma_size)");
        ...
        {
        caviar_->log.warning("lj:: (es_max_size < atom_data_type_max + 1)");
        ...
        for (auto &&e : epsilon)
        {
          ...
          {
          caviar_->log.warning("lj:: (e.size != es_max_size)");
        ...
        for (auto &&s : sigma)
        {
          ...
          {
          caviar_->log.warning("s.size != es_max_size");
      ...
      else if (cutoff_list_activated)
      {
        ...
        {
          cutoff_list.resize(max_size);
        caviar_->log.warning("lj:: (cutoff_list.size() < atom_max_size)");
        }
        for (auto &&c : cutoff_list)
        {
          if (c.size() < max_size)
          {
            c.resize(max_size, 0);
          caviar_->log.warning("lj:: (cc.size() < atom_max_size)");
          }
        }
      }
      ...
      if (lambda_s_is_set)
      {
        if (lambda_s.size() <= atom_data_type_max)
          lambda_s.resize(atom_data_type_max + 1);
        for (unsigned int i = 0; i < lambda_s.size(); ++i)
        {
          if (lambda_s[i].size() <= atom_data_type_max)
            lambda_s[i].resize(atom_data_type_max + 1, 1.0);
        }
      ...
      if (lambda_e_is_set)
      {
        if (lambda_e.size() <= atom_data_type_max)
          lambda_e.resize(atom_data_type_max + 1);
        for (unsigned int i = 0; i < lambda_e.size(); ++i)
        {
          if (lambda_e[i].size() <= atom_data_type_max)
            lambda_e[i].resize(atom_data_type_max + 1, 1.0);
        }
