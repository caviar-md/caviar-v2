...
namespace caviar2
{
  ...
  Neighborlist::Neighborlist(Caviar2 *fptr) : caviar_{fptr},
                                              atom_data{nullptr}
  {
    set_caviar(fptr);
  }
  ...
  void Neighborlist::set_caviar(caviar2::Caviar2 *c)
  {
    caviar_ = c;
    domain = &(c->domain);
  ...
  void Neighborlist::init()
  {
    ...
    if (cutoff_extra_coef < 1.0)
      caviar_->log.error_all(FC_FILE_LINE_FUNC, "cutoff_extra_coef must be equal or larger than 1.0");
        ...
        for (ind.z = 0; ind.z < no_bins.z; ++ind.z)
        {
          ...
          for (int ind_x_i = ind_x_min; ind_x_i <= ind_x_max; ++ind_x_i)
          {
            for (int ind_y_i = ind_y_min; ind_y_i <= ind_y_max; ++ind_y_i)
            {
              ...
              {
                caviar2::Vector3d<int> nvec{ind_x_i, ind_y_i, ind_z_i};
