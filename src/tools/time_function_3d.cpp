...
namespace tools
{
  ...
  Time_function_3d::Time_function_3d(Caviar2 *fptr)
  {
...
#else
    caviar_->log.error_all(FC_FILE_LINE_FUNC, "CAVIAR must be build with 'CAVIAR_WITH_MUPARSER=ON' for 'Time_function_3d'");
#endif
  }
...
/*
  bool Time_function_3d::read(caviar2::interpreter::Parser *parser)
  {
    FC_OBJECT_READ_INFO
    ...
    while (true)
    {
      ...
      else if (string_cmp(t, "export_file_append"))
      {
        ...
        caviar_->log.error_all(FC_FILE_LINE_FUNC_PARSE, "Time_function_3d creation: Unknown variable or command ");
    }
    return in_file;
    ;
  }
